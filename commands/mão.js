...
exports.run = (bot, message, args) => {
  try {
    ...
    if (!recipient) {
      const embed = new Discord.RichEmbed().setColor('#FBCFCF').setImage(hand);
      return message.channel.send(
        `Você não pode ficar de mãos dadas com você mesmo, mas eu fico de mãos dadas com você ^^, ${message.author}!`,
        embed
      );
...
exports.help = {
  name: 'mão',
  description: `\`${config.prefix}mão @membro\` faz você ficar de mãos dadas com um membro`,
};
