...
exports.run = (bot, message, args) => {
    let cargos = message.guild.roles.sort((a, b) => a.position - b.position).map(role => role.toString()).slice(1).reverse().join(" ");
    ...
    const allowed = Object.entries(permissoes).filter(([perm, allowed]) => allowed).map(([perm]) => perms[perm]).join(', ');
    ...
    const embed = new Discord.RichEmbed()
        ...
        .addField('❯\u2000\Informação', `•\u2000\**ID:** ${role.id}\n\•\u2000\**Criada em:** ${moment(role.createdAt).format('DD/MM/YYYY')}\n\•\u2000\**Posição:** ${message.guild.roles.size - role.position} de ${message.guild.roles.size}\n\•\u2000\**Membros com a role:** ${role.members.size}`, true)
        .addField('❯\u2000\Diversos', `•\u2000\**Cor:** ${role.hexColor}\n\•\u2000\**Hoisted:** ${role.hoist ? 'Yes' : 'No'}\n\•\u2000\**Mencionável:** ${role.mentionable ? 'Yes' : 'No'}\n\•\u2000\**Manipulável:** ${role.managed ? 'Yes' : 'No'}`, true)
