  ...
  try {
    ...
    if (!sign)
      return message.channel.send(
        'Insira um signo depois do comando! (capricorn, aquarius, pisces, aries, taurus, gemini, cancer, leo, virgo, libra, scorpio, sagittarius)'
      );

    if (!signs.includes(sign.toLowerCase()))
      return message.channel.send(
        'Isso aí nem é um signo (capricorn, aquarius, pisces, aries, taurus, gemini, cancer, leo, virgo, libra, scorpio, sagittarius)'
      );
    ...
    const embed = new Discord.RichEmbed()
      .setColor('#5D7B9D')
      .setAuthor(
        `Horóscopo para ${body.sunsign} em ${moment(body.date).format('DD/MM/YYYY')}`,
...
exports.help = {
  name: 'horóscopo',
  description: `Envia informações do horóscopo. Uso: \`${config.prefix}horóscopo (capricorn, aquarius, pisces, aries, taurus, gemini, cancer, leo, virgo, libra, scorpio, sagittarius)\``,
};
