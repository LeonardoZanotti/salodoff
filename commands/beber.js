  ...
  try {
    ...
    if (!bebida) {
      ...
      return message.channel.send(
        `Use \`${bot.config.prefix}beber (nome da bebida)\`. Bebidas que temos hoje: agua, bloodymary, burguesa, cafe, cha, chocolate, chopp, coca, corote, curaçau, leite, margarita, martini, mojito, monster, pinacolada, redbull, tequila, vinho, vodka e whisky.`,
        embed
      );
      ...
      const embed = new Discord.RichEmbed()
        ...
        .setImage('attachment://barman.jpg');
      return message.channel.send(
        'Desculpe, mas a casa não possui esta bebida. Bebidas que temos hoje: agua, bloodymary, burguesa, cafe, cha, chocolate, chopp, coca, corote, curaçau, leite, margarita, martini, mojito, monster, pinacolada, redbull, tequila, vinho, vodka e whisky.',
        embed
      );
    } else {
      const embed = new Discord.RichEmbed()
      ...
      collector2.on('collect', async () => {
        ...
        collector1.on('collect', async () => {
          ...
          collector.on('end', (collected) => {
            if (collected.size === 0) {
              return message.reply(`Tua bebida ${drinks[bebida]} foi roubada porque tu moscou`).then(msss.delete());
            }
          });
        });
        collector1.on('end', (collected) => {
          if (collected.size === 0) {
            return message.reply(`Tua bebida ${drinks[bebida]} foi roubada porque tu moscou`).then(mss.delete());
          }
        });
      });
      collector2.on('end', (collected) => {
        if (collected.size === 0) {
          return message.reply(`Tua bebida ${drinks[bebida]} foi roubada porque tu moscou`).then(ms.delete());
        }
      });
    }
  } catch (err) {
